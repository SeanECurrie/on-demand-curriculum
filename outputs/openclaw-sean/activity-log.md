...
| 2026-02-22 | Phase 0 overhaul: Added Steps 0.1 (Connectivity & Power Check) and 0.2 (macOS Update Check). Rewrote Step 0.3 (Identity & Sync Isolation) — disable iCloud sync individually, don't sign out of Apple ID. Community research via Bright Data informed all changes (Tailscale sleep bug, HDMI dummy plug, caffeinate). Deployment plan deduplicated to reference table. Crib sheet aligned. |
| 2026-02-22 | **Session restart skill created:** `skills/session-restart/SKILL.md` + `operator/session-state.md`. Skill handles context-efficient session startup — reads session state, loads only needed files, checks staleness, presents focused brief. CLAUDE.md Session Protocol updated to reference both files. |
| 2026-02-22 | **OpenClaw ecosystem snapshot** researched (16 Bright Data queries). Third-party tools, community reactions, lightweight alternatives mapped. Report saved external to project (not part of KB). Key findings: "nano" movement (Nanobot 22.8K stars, PicoClaw 17.6K stars), SecureClaw security tooling, monitoring dashboards, Mac-specific tooling gap. |
| 2026-02-22 | **Interactive walkthrough plan created.** Researched Claude Code agent teams (experimental feature), Excalidraw alternatives (Rough.js + Mermaid.js recommended), mapped walkthrough structure. 4-agent team designed: Design Researcher→Content Fidelity Checker (bookend), Content Architect, Diagram Engineer, Integration & UX Validator. Plan includes full agent team prompt. Location: `docs/walkthrough/interactive/`. Source→interactive sync relationship encoded in CONTEXT.md. |
| 2026-03-03 | GitHub repo created: SeanECurrie/on-demand-curriculum (private). All commits pushed. Phase 0 plan docs committed. |
| 2026-03-03 | Session restart skill invoked. Session brief: 8 days since last session, staleness flag raised. |
| 2026-03-03 | **On-Demand Curriculum Engine brainstorming session** (superpowers:brainstorming). Catalyst: friend Jeff asking about OpenClaw — business owner, AI-new, wants dedicated MacBook Air. Identified opportunity to productize the research process. |
| 2026-03-03 | 4 parallel audit agents dispatched to classify every file in the repo as engine/output/DNA. Agents read: CLAUDE.md, CONTEXT.md, all operator docs, all 7 KB buckets, 5 research reports, all patterns, full walkthrough (2,381 lines), interactive HTML (3,030 lines), style guide (1,037 lines), all plan docs, all logs. |
| 2026-03-03 | **Design doc written:** `docs/plans/2026-03-03-on-demand-curriculum-engine-design.md`. Three-layer architecture (DNA + Engine + Outputs), 5-stage pipeline (intake → research → synthesis → output → delivery). |
| 2026-03-03 | **Implementation plan written:** `docs/plans/2026-03-03-on-demand-curriculum-engine-implementation.md`. 17 tasks across 5 phases (audit → extract → relocate → constitute → verify) with 4 mandatory review checkpoints. 1,080 lines. |
